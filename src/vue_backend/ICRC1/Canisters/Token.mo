...
shared ({ caller = _owner }) actor class Token(
    ...
    let appCanister = actor("be2us-64aaa-aaaaa-qaabq-cai") : actor {
    ...
    public shared ({ caller }) func donate(args : ICRC1.TransferArgs, donationArgs: FundRaiseTypes.DonationArgs) : async ICRC1.TransferResult {
        var result : ICRC1.TransferResult = await* ICRC1.transfer(token, args, caller);

        switch(result){
            case(#Ok(txIndex)){
               var x:Text = await appCanister.donate(caller, txIndex, donationArgs);
            };
            case (#Err(_)) {};
        };
        result
