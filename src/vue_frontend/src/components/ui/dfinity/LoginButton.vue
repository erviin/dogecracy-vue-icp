...
<template>
    <Button v-if="!IcpStore.isAuthenticated" :disabled @click="login"  class="text-black hover:bg-black/80 hidden md:flex ">
        ...
        <DropdownMenu v-else-if="IcpStore.isAuthenticated">
            ...
            <DropdownMenuContent class="w-56 px-0 py-4 border-0 bg-black " data-align="left">
                <DropdownMenuItem class="p-0">
                    <RouterLink to="/profile" class="flex text-white items-center space-x-2 w-full h-full p-4 bg-black hover:bg-black/80 rounded-none  text-md font-intro">
                        <User2Icon />
                        <span>Your Profile</span>
                    </RouterLink>
                </DropdownMenuItem>
                <DropdownMenuItem class="p-0">
                    <Button @click="logout" class="w-full p-4 h-full bg-black hover:bg-black/80  rounded-none  text-md font-intro">
